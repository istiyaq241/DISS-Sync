...
export default function ClassEdit({ schedules, onSaved }) {
  ...
  const saveSessionsToFirestore = async (updated) => {
    ...
    try {
      ...
      const snap = await getDocs(q);
      const record = { teacherName: attendTeacher, date: todayStr, sessions: updated };
      if (!snap.empty) {
  ...
  const adjustToken = async (studentName, delta) => {
    ...
    try {
      if (tokenData[studentName]?.docId) {
        await updateDoc(doc(db, 'tokens', tokenData[studentName].docId), { tokens: next });
        setTokenData((prev) => ({ ...prev, [studentName]: { ...prev[studentName], tokens: next } }));
      } else {
        const newRef = await addDoc(collection(db, 'tokens'), {
          classNo: tokensClass, studentName, date: dateStr, tokens: next,
        });
